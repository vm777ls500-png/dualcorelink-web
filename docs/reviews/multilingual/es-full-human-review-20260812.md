# Spanish Full Human Review Packet
...
- Internal candidate targets use `/es/`; broken, English fallback, wrong-locale, and query targets: 0 in the targeted audit.
...
For every row in [es-full-decisions-20260812.md](./es-full-decisions-20260812.md):
...
- Verify natural business Spanish and exact product specifications.
...
|---|---|---|---|---|---|
| es | 69 candidates | Composition | Generic publication fallback on specialized routes | Shared specialized Product, Solution, Resource, Region, About, Contact, FAQ, and listing composition | Preserve semantic, media, and relationship parity |
| es | Shared UI | Labels and paths | English fallback labels or paths | Candidate-locale labels and `/es/` review paths | Remove preview-only fallback defects |
| es | Candidate facts | Factual payload | Existing candidate facts | Unchanged | Technical preparation does not rewrite facts |
