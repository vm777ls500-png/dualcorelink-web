# Persian Full Human Review Packet
...
- Internal candidate targets use `/fa/`; broken, English fallback, wrong-locale, and query targets: 0 in the targeted audit.
- Direction: `rtl`; Persian reuses the established RTL and bidi architecture.
...
For every row in [fa-full-decisions-20260812.md](./fa-full-decisions-20260812.md):
...
- Verify natural business Persian and exact product specifications.
...
|---|---|---|---|---|---|
| fa | 69 candidates | Composition | Generic publication fallback on specialized routes | Shared specialized Product, Solution, Resource, Region, About, Contact, FAQ, and listing composition | Preserve semantic, media, and relationship parity |
| fa | Shared UI | Labels and paths | English fallback labels or paths | Candidate-locale labels and `/fa/` review paths | Remove preview-only fallback defects |
| fa | Candidate facts | Factual payload | Existing candidate facts | Unchanged | Technical preparation does not rewrite facts |
