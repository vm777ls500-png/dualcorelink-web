...
final class DualCoreLink_Multilingual_Import_Service
{
    ...
    ): array
    {
        ...
        foreach ($payload as $record) {
            ...
            if ($policy['batch'] === 'remaining-final' &&
                ($record['priority'] ?? null) !== ($approved['priority'] ?? null)) {
                $errors[] = "priority whitelist mismatch: {$id}";
            }
            $expected_group = "shb2b-{$record['contentType']}-{$id}";
            if (array_key_exists('translationGroup', $record) &&
                $record['translationGroup'] !== $expected_group) {
                $errors[] = "translation group mismatch: {$id}";
            }
