...
test("all candidates retain evidence while only reviewed pages enter production outputs", () => {
  ...
  assert.deepEqual(
    ...
    new Set(multilingualPublicationManifest.map((entry) => entry.localizedUrl)),
  );
});
...
test("CMS import payload uses verified source IDs without invented localized IDs", () => {
  ...
  for (const payload of cmsTranslationImportPayload) {
    ...
    if (zhReviewedReleaseUrls.includes(localizedUrl as (typeof zhReviewedReleaseUrls)[number])) {
      ...
      assert.equal(payload.nativeReviewStatus, "approved");
      assert.equal(payload.nativeReviewer, "Allan");
      assert.equal(payload.nativeReviewDate, "2026-08-12");
    }
  }
});

test("full release check accepts all 414 reviewed localized pages", async () => {
  ...
  assert.equal(result.productionReleaseReadyCount, 414);
  assert.equal(result.cmsPayloadCount, 252);
  assert.equal(result.cmsPayloadStructurallyReadyCount, 252);
  assert.equal(result.cmsPayloadNativeApprovedCount, 252);
...
test("canonical and hreflang are self-consistent and reference only published pages", () => {
  ...
  assert.ok(shared.zh);
  assert.ok(shared.ar);
  assert.equal(shared.de, "https://dualcorelink.com/de/about/");
  assert.equal(shared.es, "https://dualcorelink.com/es/about/");
  assert.equal(shared.vi, "https://dualcorelink.com/vi/about/");
  assert.equal(shared.fa, "https://dualcorelink.com/fa/about/");
});
...
test("navigation exposes only released localized paths", () => {
  for (const localizedUrl of zhReviewedReleaseUrls) {
    ...
    assert.deepEqual(getReleasedLocalesForPath(contentPath), ["zh", "ar", "vi", "de", "es", "fa"]);
    assert.equal(
    ...
    for (const locale of ["de", "es", "fa"]) {
      assert.equal(isReleasedLocalizedPath(locale, contentPath), true);
      assert.equal(
  ...
  assert.deepEqual(getReleasedLocalesForPath("regions"), ["zh", "ar", "vi", "de", "es", "fa"]);
  assert.equal(
...
test("localized page lookup exposes all released language pages", () => {
...
test("all reviewed localized pages are served through exact allowlists", async () => {
  ...
  assert.match(
    nginx,
    /Native-reviewed multilingual release batches: 69 URLs for each released locale\./,
  );
  assert.match(nginx, /\^\/\(\?<reviewed_locale>zh\|ar\|vi\|de\|es\|fa\)\/\(\?<reviewed_path>about\|contact\|faqs/);
  assert.match(
    nginx,
  ...
  assert.match(
    nginx,
    /\^\/\(\?<reviewed_rsc_locale>zh\|ar\|vi\|de\|es\|fa\)\/\(\?<reviewed_rsc_path>about\|contact\|faqs/,
  );
  assert.match(
    nginx,
